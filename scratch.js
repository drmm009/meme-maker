...
const formattedTemplates = newTemplates.map(t => {
  // Generate generic default captions based on box_count
  const defaultCaptions = [];
  for (let i = 0; i < t.box_count; i++) {
    // Top text and bottom text generally for 2 box count
    let y = 0.1;
    if (i === 1 && t.box_count === 2) y = 0.8;
    else if (i > 0) y = 0.2 + (i * 0.15); // Stagger for multiple boxes
  ...
  return {
    id: t.id.toString(),
    name: t.name,
    category: 'popular', // Or trending
    tag: 'free',
    imageUrl: t.url,
    defaultCaptions: defaultCaptions,
    width: t.width,
    height: t.height,
    trendingScore: Math.min(100, Math.floor(t.captions / 10000)) // Fake trending score based on captures
  };
});

// Read existing templates.js
let content = fs.readFileSync('src/data/templates.js', 'utf8');

// Find where export const MEME_TEMPLATES = [ is
const insertIndex = content.indexOf('export const MEME_TEMPLATES = [') + 'export const MEME_TEMPLATES = ['.length;

// Convert formattedTemplates to JS string
let newTemplatesStr = '';
formattedTemplates.forEach(t => {
  newTemplatesStr += \\n  {\n    id: '\',\n    name: '\',\n    category: '\',\n    tag: '\',\n    imageUrl: '\',\n    defaultCaptions: [\n\;
  t.defaultCaptions.forEach((c, idx) => {
    newTemplatesStr += \      { text: '\', x: \, y: \, width: \, height: \, fontSize: \, color: '\', stroke: '\', align: '\' }\\n\;
  });
  newTemplatesStr += \    ],\n    width: \,\n    height: \,\n    trendingScore: \\n  },\;
});

// Insert new templates at the beginning of the array
content = content.slice(0, insertIndex) + newTemplatesStr + content.slice(insertIndex);

fs.writeFileSync('src/data/templates.js', content);
console.log('Added ' + formattedTemplates.length + ' templates to src/data/templates.js');
