...
import { Video, Type, Music, Image as ImageIcon, Smile } from 'lucide-react';
...
export default function Timeline() {
  ...
  const handleAddEmojiSticker = (emoji) => {
    ...
    useEditorStore.getState().addItem({ type: 'sticker', url, name: emoji + ' Sticker' });
  ...
  const handleAddText = () => {
    useEditorStore.getState().addItem({ type: 'text', text: 'New Text', durationMs: 3000, name: 'Text Block' });
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', width: '100%', flex: 1, minHeight: 0 }}>
      ...
      <div 
        ...
        style={{ height: '30px', borderBottom: '1px solid var(--border-light)', position: 'sticky', top: 0, zIndex: 50, background: 'var(--bg-surface)', cursor: 'ew-resize', flexShrink: 0, marginLeft: '64px', marginRight: '15px' }}
      >
